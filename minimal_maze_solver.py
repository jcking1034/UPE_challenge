...
for i in range(5):
    ...
    maze, move_list, maze_stack = [[1 if i == cur_loc[1] and j == cur_loc[0] else 0 for i in range(new_body['size'][1])] for j in range(new_body['size'][0])], [], []
    while True:
        if cur_loc[0] + 1 < new_body['size'][0] and maze[cur_loc[0] + 1][cur_loc[1]] == 0 and (cur_loc[0] + 1, cur_loc[1]) not in maze_stack:  maze_stack.append((cur_loc[0] + 1, cur_loc[1]))
        ...
        if cur_loc[1] + 1 < new_body['size'][1] and maze[cur_loc[0]][cur_loc[1] + 1] == 0 and (cur_loc[0], cur_loc[1] + 1) not in maze_stack:  maze_stack.append((cur_loc[0], cur_loc[1] + 1))
