...
for i in range(5):
    ...
    while True:
        ...
        while abs(next_spot[0] - new_body['cur_loc'][0]) + abs(next_spot[1] - new_body['cur_loc'][1]) > 1:
            ...
            new_body['cur_loc'][{'up': 1, 'down': 1, 'right': 0, 'left': 0}[last_move]], body = new_body['cur_loc'][{'up': 1, 'down': 1, 'right': 0, 'left': 0}[last_move]] - {'up': -1, 'down': +1, 'right': +1, 'left': -1}[last_move], requests.post('http://ec2-34-211-81-131.us-west-2.compute.amazonaws.com' + '/game?token=' + requests.post('http://ec2-34-211-81-131.us-west-2.compute.amazonaws.com' + '/session', data = {'uid': '205125796'}).json()['token'], data = {'action': {'up': 'down', 'down': 'up', 'right': 'left', 'left': 'right'}[last_move]}).json()
